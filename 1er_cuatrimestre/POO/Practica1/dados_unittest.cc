...
TEST(Dados, getUltimos1) {
  ...
  d.getUltimos1(v1);
  d.getUltimos2(v2);
  for (i=1;i<6;i++) EXPECT_EQ(i,v1[i-1]);
  for (i=1;i<6;i++) EXPECT_EQ(i,v2[i-1]);
}
...
TEST(Dados, getUltimos2) {
  ...
  d.getUltimos1(v1);
  d.getUltimos2(v2);
  for (i=0;i<5;i++) EXPECT_EQ(v1t[i],v1[i]);
  for (i=0;i<5;i++) EXPECT_EQ(v2t[i],v2[i]);
}
...
TEST(Dados, getUltimos3) { // Otro buen test con random()
  ...
  d.getUltimos1(v1b);
  d.getUltimos2(v2b);
  for (i=0;i<5;i++) EXPECT_EQ(v1a[i],v1b[i]);
  for (i=0;i<5;i++) EXPECT_EQ(v2a[i],v2b[i]);
}
