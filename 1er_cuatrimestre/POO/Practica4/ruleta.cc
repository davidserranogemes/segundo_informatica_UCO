...
int Ruleta::getdineroEnJuego_(){
	int dineroEnJuego=0;
	list<Jugador>::iterator i;

	for(i=jugadores_.begin();i!=jugadores_.end();i++){
		dineroEnJuego+=i->getDinero();
	}
	dineroEnJuego+=getBanca();
	return (dineroEnJuego);
}


void Ruleta::getEstadoRuleta(int &numJugadores,int &dineroEnJuego,int &lanzamientos, int &beneficio){
		numJugadores=jugadores_.size();
		dineroEnJuego=getdineroEnJuego_();
		beneficio=getBanca()-1000000;
		lanzamientos=getLanzamientos_();
}



}//Llave de cierre de namespace
